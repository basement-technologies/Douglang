...
loop [
	...
	Doug -set 1
    prediction (Doug Doug) = 0 [believers win [end stream]]
]
tts "LAUNDRY!!!!!!!" 
```
...
```douglang
five minute coding adventure add [
    DougDougDoug Doug set 0
    +set (Doug DougDoug) +set (DougDoug DougDougDoug)
    guoD (Doug Doug)
]

add Doug set 1 Doug set 1 Bald Doug set call add tts 
```
